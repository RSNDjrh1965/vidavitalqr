...
**Pendiente:** el usuario debe generar el token nuevo desde el portal del BCCR (`Mi perfil → Generar token`, distinto al token de suscripción del servicio viejo que ya tiene) y agregarlo en Netlify como `BCCR_API_TOKEN`; luego subir estos dos archivos (`lib/bccr.js`, `actualizar-tipo-cambio.js`) y volver a correr "Run now" para confirmar que la API nueva sí responde bien.
