...
**Registro en el BCCR completado y primera prueba real (2026-09-23):** el usuario se registró con éxito (correo `jherrerah@cfia.or.cr`) en el portal correcto (`https://gee.bccr.fi.cr/Indicadores/Suscripciones/UI/Suscripcion` — el primer enlace que se le dio, `frmServiciosWebHermes.aspx`, estaba mal). Configuró `BCCR_CORREO`/`BCCR_TOKEN` en Netlify y subió todo el código (ZIP completo del sitio, incluyendo `netlify.toml`) a producción — el deploy quedó exitoso y Netlify reconoció correctamente `actualizar-tipo-cambio` como función programada ("Next execution on Sep 24 at 9:00 AM").

Al probarla manualmente con el botón "Run now" del panel de Netlify (nota: las funciones programadas NO se pueden invocar por URL directa, Netlify lo bloquea con 403 a propósito — hay que usar ese botón, o esperar a la ejecución programada), la función corrió bien pero el BCCR respondió **error 503** en 5 intentos seguidos. Se corrigió `lib/bccr.js`: el nombre de método usado (`ObtenerIndicadoresEconomicosXML`) se cambió a `ObtenerIndicadoresEconomicos` (sin el sufijo "XML", que es el que se confirmó funcionando en un ejemplo real de integración), y se agregó un encabezado de navegador (`User-Agent`/`Accept`) por si el BCCR bloquea peticiones sin apariencia de navegador. También se mejoró el mensaje de error para incluir un fragmento del cuerpo de la respuesta del BCCR, y así poder diagnosticar mejor si vuelve a fallar. El usuario subió el archivo corregido y volvió a correr "Run now": el error 503 persistió, pero ahora con el detalle del cuerpo de la respuesta visible en el log.

**Diagnóstico del 503 completado (2026-09-23):** el cuerpo de la respuesta del BCCR es una página genérica de error de IIS ("Service Unavailable" / "HTTP Error 503. The service is unavailable"), no una página de bloqueo de firewall/WAF. Para aislar la causa, se le pidió al usuario que pegara la URL exacta (con su correo y token reales) directo en su propio navegador, fuera de Netlify — dio el mismo error 503, idéntico. Esto descarta que sea un bloqueo por IP de los servidores de Netlify: el servicio le falla a cualquiera en ese momento, sin importar desde dónde se consulte. Se verificó además, por búsqueda web, que la URL, el nombre del método (`ObtenerIndicadoresEconomicos`) y los parámetros usados coinciden exactamente con la documentación oficial y con ejemplos de integración reales de terceros — no hay ningún error en la URL ni en la forma de armar la petición. La página general del servicio (`wsindicadoreseconomicos.asmx`, sin invocar ninguna operación) sí carga normal.

**Conclusión:** el problema es una falla/inestabilidad intermitente del lado del servidor del BCCR, específica de esa operación, no del código ni de la configuración de VidaVitalQR. Es coherente con varios 503 observados anteriormente en otras páginas de `gee.bccr.fi.cr` durante la investigación del enlace de registro.

**Migración a la API nueva del BCCR (SDDE) — 2026-09-23:** investigando más, se encontró que el BCCR tiene una segunda API, más moderna (REST/JSON, alojada en `apim.bccr.fi.cr`, con autenticación por token "Bearer" en vez de correo+token), documentada en su PDF oficial "Estándar API SDDE", y usada activamente por integraciones de terceros encontradas en GitHub — a diferencia del servicio viejo (SOAP/ASMX en `gee.bccr.fi.cr`), que corre en un servidor distinto (más antiguo, aparentemente más inestable). Esta API nueva sí acepta el mismo código de indicador (318 = venta) que ya usábamos, a través del endpoint `/indicadoresEconomicos/{codigo}/series`.

Se reescribió `lib/bccr.js` con dos métodos: la API nueva como preferida (`obtenerTipoCambioVentaAPINueva`, requiere la variable de entorno nueva `BCCR_API_TOKEN`) y el servicio viejo como respaldo automático (`obtenerTipoCambioVentaServicioViejo`, sigue usando `BCCR_CORREO`/`BCCR_TOKEN`) — si la API nueva falla o no está configurada, se intenta automáticamente con la vieja, y solo si ambas fallan se reporta el error (con el detalle de ambos intentos). `actualizar-tipo-cambio.js` se actualizó para leer y pasar `BCCR_API_TOKEN` además de las variables anteriores.

**Confirmado funcionando en producción (2026-09-23):** el usuario generó el token nuevo en el BCCR, lo agregó en Netlify como `BCCR_API_TOKEN`, subió el código y corrió "Run now". Log resultante: `oficial(venta)= 451.86 → con margen (2%, redondeado hacia arriba)= 461` — el valor 451.86 coincide exactamente con el tipo de cambio de venta que el propio BCCR mostraba ese día en su portal público, confirmando que la API nueva trae el dato correcto. Sin ningún error. El tipo de cambio con margen (₡461) quedó guardado en `config/tipo-cambio.json` en S3, listo para que lo usen `crear-pago.js` y el selector de moneda del front-end.

**Activación y primera transacción de prueba real en CRC (2026-09-23):** el usuario agregó `HABILITAR_PAGO_CRC=true` en Netlify (tuvo que hacer un nuevo deploy además de guardar la variable, para que la función la tomara) y completó un pago de prueba real con ONVO Pay (modo test, sin dinero real) por "Renovación anual VidaVitalQR — Persona" ($7.91 con IVA, tipo de cambio ₡461, total esperado ₡3,647). **Resultado de la primera prueba:** ONVO Pay cobró "CRC 36.47" en vez de "CRC 3,647" — es decir, interpretó el monto 100 veces más pequeño de lo esperado. Se confirmó revisando directamente el detalle de la transacción en ONVO Pay (no solo el correo de aviso, que no mostraba el monto real cobrado).

**Diagnóstico y corrección:** ONVO Pay espera el monto en la subunidad más pequeña de la moneda (céntimos) también para colones — igual que ya hacía para dólares (centavos) — y no en la unidad entera como se había asumido. Se corrigió `crear-pago.js`: el valor por defecto de `ONVO_CRC_SUBUNIT_MULTIPLIER` cambió de `1` a `100` (el total en colones se sigue calculando siempre como un entero — `Math.ceil`, con el margen del 2% — lo único que cambió es que ahora se multiplica por 100 antes de mandarlo a ONVO Pay). Ya no depende de tener que configurar una variable de entorno aparte para que quede bien: el valor correcto es ahora el que trae por defecto.

**Verificado:** sintaxis de `crear-pago.js` (`node -c`) sin errores. **Pendiente:** subir este archivo corregido y repetir la transacción de prueba una vez más, confirmando esta vez en el detalle de ONVO Pay que el monto cobrado sea exactamente "CRC 3,647.00" (o el monto correcto del día) antes de considerar el pago en colones completamente listo para uso real por clientes.

**Verificado:** sintaxis de ambos archivos (`node -c`) sin errores. No fue posible probar la respuesta real de la API nueva en este entorno de trabajo (requiere el token del usuario), así que el parseo del JSON se hizo con base en la estructura documentada oficialmente por el BCCR (campos `estado`/`mensaje`/`datos[].series[].fecha`/`valorDatoPorPeriodo`) — si el formato real difiriera, el mensaje de error ahora incluye siempre un fragmento del cuerpo de la respuesta cruda para poder ajustarlo rápido.
...
## 59. Corrección: impresión de etiqueta de envío torcida y bloqueo del navegador al imprimir (2026-09-23)

Retomando la observación 2 del punto 55 (quedó pendiente de diagnóstico en ese momento): el usuario probó "Imprimir etiqueta" en `admin-envios.html` con su impresora térmica (MHT-L1081) y la etiqueta salió torcida/mal alineada; además, después de intentar imprimir, el navegador quedaba bloqueado/sin responder. La descarga como imagen PNG (botón alterno, ya existente desde el punto 42) sigue funcionando perfecta, cargándola directo en el software propio de la impresora.

**Causa del bloqueo (confirmada por revisión del código):** el botón cerraba la ventana de impresión con un `setTimeout` fijo de solo 300 milisegundos después de llamar a `print()`, sin esperar a que el diálogo de impresión del navegador realmente terminara. Con esta impresora, ese diálogo tarda más, así que la ventana se cerraba mientras el navegador todavía la estaba usando — eso es lo que dejaba todo bloqueado.

**Causa de que salga torcida (diagnóstico, no verificable sin la impresora física):** el intento anterior (punto 55/18-sep) giraba la etiqueta 90° por CSS para calzar con la orientación vertical del rollo de la impresora, pero el diálogo de impresión del navegador de esta impresora térmica no siempre respeta el tamaño de papel indicado por la página (`@page`) — a diferencia de "Guardar como PDF", con una impresora física real el tamaño de papel casi siempre lo decide el propio diálogo/driver de impresión, no la página. Si ese tamaño no coincide exactamente, el navegador escala o recorta el contenido ya rotado, y el resultado sale torcido.

**Corrección aplicada:**
1. Se corrigió el bloqueo de raíz: en vez de un `setTimeout` fijo de 300ms, ahora se usa el evento estándar `afterprint` (se dispara cuando el usuario cierra o confirma el diálogo de impresión, sin importar cuánto tarde), con un respaldo de 15 segundos (antes 300ms) por si algún navegador no lo dispara — de sobra para revisar el diálogo con calma antes de que se cierre solo.
2. Se reordenaron los botones: "Descargar como imagen (PNG) — recomendado" ahora es el principal (es el método que el usuario confirmó que funciona siempre bien), y "Imprimir etiqueta (desde el navegador)" queda como alternativa secundaria.
3. Al usar "Imprimir etiqueta", ahora aparece una nota explicando qué revisar manualmente en el diálogo de impresión del navegador para que no salga recortada/torcida: tamaño de papel personalizado (2 x 3 in / 50.8 x 76.2 mm) y escala 100% (no "Ajustar al papel") — y sugiere usar mejor la descarga en PNG si hay dudas.

**No verificable en este entorno de trabajo:** no hay forma de probar contra la impresora térmica real (MHT-L1081) desde aquí, así que la corrección de la torcedura es un diagnóstico razonado, no una prueba confirmada — el arreglo del bloqueo (causa del setTimeout) sí es una corrección de un bug concreto e identificado en el código, esa parte sí queda resuelta con certeza.
...
## 61. Configuración completa de WhatsApp Business (2026-09-23 y 2026-09-24)

Sesión dedicada a dejar WhatsApp Business listo como canal de venta, sin tocar código del sitio (temas de configuración de la cuenta/app, no del repositorio).

**Catálogo y precios en colones:** el usuario ya había cargado el catálogo con precios en colones y preguntó si se podían editar o había que borrar y volver a cargar. Se confirmó que sí se pueden editar directamente (tanto desde la app móvil como desde Meta Commerce Manager), sin necesidad de borrar nada; para cambios masivos de moneda se explicó también la opción de exportar/editar/reimportar por CSV desde Commerce Manager.

**Perfil de empresa — dirección no guardaba:** el formulario de dirección del perfil solo mostraba sugerencias de Estados Unidos y no guardaba. Se diagnosticó como un problema de región del dispositivo (el buscador de direcciones usa el idioma/región del sistema); se le indicó revisar la región del teléfono, el permiso de ubicación de la app y el país configurado en Meta Business Manager. El usuario resolvió el perfil de empresa y el catálogo por su cuenta poco después.

**Formulario "Primeros pasos" de whatsapp.com:** se aclaró que es un formulario de contacto/información opcional de Meta (no el registro real de la cuenta), y se ayudó a completar los campos restantes (sede central: Costa Rica).

**Respuestas rápidas:** se redactaron y entregaron 4 respuestas rápidas listas para cargar (atajos `/precios`, `/comofunciona`, `/envio`, `/pago`), con el texto completo de cada una, para usar tanto desde la app móvil como desde la versión web/escritorio (donde no está disponible el mensaje de bienvenida/ausencia, solo en la app móvil).

**Mensaje de bienvenida y de ausencia:** se explicaron los pasos para configurarlos desde la app móvil (Herramientas de empresa → Mensaje de bienvenida / Mensaje de ausencia), con texto sugerido para ambos.

**Enlace wa.me:** se armó primero un enlace manual (`https://wa.me/50660933090?text=...`) y luego el usuario encontró y usó el enlace corto oficial que genera la propia app (`https://wa.me/message/CDRY7MKH5W52E1`, vinculado a su número de cuenta +506 6093 3090) — con la ventaja de que ese enlace no cambia aunque se edite el mensaje predeterminado. Se probó de punta a punta (abre WhatsApp con el mensaje ya escrito) y se confirmó que el botón de WhatsApp ya existente en `index.html` también funciona correctamente y dirige los mensajes al número de cuenta correcto (una prueba inicial generó una falsa alarma por aparecer un chat con un número distinto — +506 6090 3080 —, aclarada como el propio número personal del usuario usado para probar el botón, no un error de configuración).

**Estado:** WhatsApp Business queda configurado y verificado de punta a punta (perfil, catálogo, respuestas rápidas, enlace/botón de contacto). Pendiente, si el usuario lo pide más adelante: cargar el mensaje de bienvenida/ausencia (se le dio el texto sugerido, falta confirmar que lo aplicó desde el celular).

## 62. Configuración de Facebook e Instagram, y pantalla de marca + botón de llamada al abrir la ficha (2026-09-24)

**Facebook e Instagram:** se guió al usuario paso a paso (con capturas de pantalla) para crear desde cero la Página de Facebook de VidaVitalQR (foto, portada, dirección, botón de CTA, invitar amigos — incluyendo la confusión de Facebook entre modo "perfil personal" y modo "administrador de Página" para poder usar "Invitar a amigos"), publicar el primer post, y luego crear la cuenta de Instagram (registro por número de teléfono, conversión a cuenta Empresa, vinculación a la Página de Facebook vía Centro de cuentas de Meta, conexión del número de WhatsApp Business como opción de contacto del perfil) y publicar la primera foto. Ambas redes quedaron activas y conectadas entre sí y con WhatsApp Business. Pendiente: TikTok (mismo orden ya acordado con el usuario: WhatsApp → Facebook → Instagram → TikTok).

**Idea de contenido — "familiarización con el escaneo":** el usuario planteó publicar contenido (texto, imágenes, video, sonido) enfocado en normalizar el gesto de escanear el código QR. Se le recomendó: repetición del mismo gesto en distintos contextos (formato Reels/TikTok como el más efectivo), un sonido de marca reconocible en el momento del escaneo, texto/imagen estática como refuerzo (no protagonista), y una llamada a la acción física invitando a probar el escaneo por curiosidad. Se integra como parte del pilar "educativo" ya definido en `redes-sociales-vidavitalqr.md`.

**Pantalla de marca (logo + sonido) al escanear — implementada en código.** A partir de esa idea, se probó en una maqueta interactiva (fuera del repositorio) el concepto de mostrar el logo con una animación breve y un sonido al abrir la ficha, y se terminó implementando de verdad en `netlify/functions/ver.js` (la función que genera la página pública que se ve al escanear el código, para fichas de persona, mascota y objeto):

- Se agregó un overlay (`div#splashMarca`) que se muestra 1.6 segundos al cargar la página, con el logo real de VidaVitalQR (incrustado como imagen en base64, sin depender de archivos externos) apareciendo con una animación de escala/desvanecido, y luego se oculta solo, revelando la ficha normal debajo (que ya estaba renderizada desde el primer momento, sin bloquear nada).
- El sonido se genera por código con Web Audio API (osciladores, sin archivos de audio externos): la opción elegida por el usuario fue "campanita suave extendida" (varias notas melódicas con resonancia larga, ~1.5-1.8s), tras comparar varias alternativas (acorde ascendente, beep de escáner, latido, barrido digital, y un "monitor de hospital/ECG" que se descartó explícitamente por su asociación cultural con el momento en que se detiene el corazón — transmite lo opuesto a lo buscado).
- Todo el mecanismo de marca es puramente decorativo y está envuelto en `try/catch`: si el navegador bloquea el audio (común en iPhone o con el teléfono en silencio) o falla por cualquier motivo, la ficha se muestra exactamente igual, solo sin sonido. No modifica en nada la lógica existente de geolocalización ni el aviso por correo a los contactos de emergencia (`avisar-escaneo.js`), que sigue disparándose exactamente igual que antes, desde el navegador, después del permiso de ubicación.

**Botón de llamada con número enmascarado.** Se agregó, junto a cada contacto de emergencia con teléfono, un botón "Llamar" (enlace `tel:`) que abre el marcador del teléfono con el número real precargado, pero cuyo texto visible muestra el número enmascarado (solo los últimos 3 dígitos, ej. `*****000`) — a petición explícita del usuario, para que el número no quede expuesto con solo mirar el botón. El número completo se sigue mostrando igual que antes en el texto de la ficha (`Tel. 8888-0000`), ya que esa parte solo la ve quien abre y lee la ficha con atención. Se agregó también la traducción de "Llamar" en los 4 idiomas ya soportados (ES/EN/FR/PT).

**Decisión: la ficha NO debe quedar disponible offline.** El usuario preguntó si la ficha queda guardada permanentemente en el celular tras escanearla. Se confirmó que, por diseño, no queda guardada ni instalada (no hay caché offline, no hay datos en almacenamiento local del navegador) — cada apertura requiere internet y carga desde cero, tal como ya funcionaba. Se descartó explícitamente implementar una versión PWA/offline (idea que se había sugerido inicialmente como mejora de confiabilidad, pero que el usuario no quiere, precisamente para que solo se pueda volver a ver la ficha escaneando el código de nuevo). Única salvedad aclarada al usuario, y que no depende del sitio: el navegador del celular puede guardar la URL visitada en su propio historial, igual que con cualquier página web.

**Verificación realizada:** se probó el renderizado de `ver.js` con datos de ejemplo para los tres tipos de ficha (persona, mascota, objeto), confirmando por código y con capturas de pantalla (splash con el logo real, y la ficha completa después) que: el overlay y el botón de llamada aparecen correctamente, el número de teléfono sigue mostrándose completo en el texto de la ficha, los enlaces `tel:` quedan con el número completo (con prefijo `+506` cuando el número es local de 8 dígitos), y que la lógica de ubicación/aviso por correo a los contactos (`avisarEscaneo`, los botones de "Sí/No compartir ubicación exacta") permanece intacta. Se comparó además un diff completo contra la versión anterior del archivo para confirmar que no se modificó nada más.

**Nota sobre documentos guardados en el Proyecto de Claude:** las decisiones de esta sesión (sonido elegido y descartados, botón de llamada enmascarado, decisión de no-offline) quedaron documentadas también en `claude/pantalla-sonido-marca-vidavitalqr.md` dentro del Proyecto de Claude, como referencia para sesiones futuras.

**Pendiente:** usar la misma pieza de sonido/animación como recurso de contenido en los videos cortos de redes sociales (parte del pilar "familiarización con el escaneo"); seguir con la configuración de TikTok.

## 63. Cuenta ONVO Pay aprobada — se retira la etiqueta "Mockup de checkout" en `index.html` (2026-09-24)

El usuario confirmó que ONVO Pay ya aprobó su cuenta y que ya tiene en su poder las llaves de **producción** (correo de confirmación de ONVO revisado: además de las llaves, ONVO pide enviar a `ayuda@onvopay.com` uno de estos documentos para que las liquidaciones no tengan contratiempos — estado de cuenta bancario de los últimos 3 meses, tres últimas declaraciones del IVA, o declaración de renta del último período; pendiente de que el usuario lo gestione directamente con ONVO, no es algo que se resuelva desde el código). A partir de ahí, el usuario pidió "poner a funcionar" el sitio y dejó ver que quería empezar por "actualizar el mockup".

Se revisó el checkout de `index.html` y se confirmó que el flujo de pago real con ONVO Pay ya estaba implementado desde antes (puntos 31-40): cuando el carrito tiene folio conocido para cada renovación seleccionada (o solo trae productos físicos/digitales nuevos, que no necesitan folio), el formulario activa `modoReal` y redirige a ONVO Pay de verdad — lo que quedaba desactualizado era únicamente el **texto** de la sección, que en varios puntos fijos seguía diciendo "Mockup de checkout — solo demostración, no procesa pagos reales", aunque el pago detrás ya fuera real (en modo prueba). Se actualizó ese texto en los 4 idiomas soportados (ES/EN/FR/PT):

- Se quitó el aviso fijo en la parte superior de la sección ("Mockup de checkout — solo demostración, no procesa pagos reales").
- El rótulo ("eyebrow") sobre el título pasó de "Mockup de compra — solo demostración" a "Pago seguro con ONVO Pay".
- La etiqueta junto al candado pasó de "Pago seguro (demo)" a "Pago seguro".
- El texto introductorio pasó de condicional ("Así se vería el pago...") a afirmativo ("Así se procesa el pago...").
- El título y la primera respuesta de "Preguntas frecuentes" pasaron de "Sobre este mockup" / "No, es una demostración visual..." a "Preguntas frecuentes sobre el pago" / "Sí. Esta sección procesa el pago de forma real y segura a través de ONVO Pay...".
- Se quitó la palabra "mockup" de la respuesta sobre cómo se actualiza el resumen del pedido.
- Se corrigió además un detalle que no se había notado hasta ahora: el aviso "Mockup de checkout — demostración únicamente..." que aparece **dentro** del recuadro de pago (el que si es correcto que exista, porque cubre el caso real de una renovación seleccionada sin folio identificado todavía, donde el sitio cae a un formulario de repaso para no bloquear al cliente) se mostraba por defecto al cargar la página, antes de seleccionar cualquier producto, porque nada lo ocultaba en ese estado inicial. Se corrigió para que empiece oculto y solo aparezca si de verdad se cae a ese caso de repaso — ya no se ve al entrar a la página ni al elegir un producto normal.
- **No se tocó nada de la lógica de pago:** la detección de `modoReal`, la creación de la sesión de pago con ONVO, el regreso con `?pago=exitoso|cancelado`, el aviso de pago real al administrador, ni el cálculo de IVA/envío — todo eso permanece exactamente igual.

**Verificación realizada:** comparación completa (diff) contra la copia de respaldo del archivo antes de editar, confirmando que los únicos cambios están en las líneas de texto señaladas arriba (más el ajuste de visibilidad del aviso interno). Render con Playwright de la sección de checkout en tres estados — carrito vacío (ya no muestra "Mockup"), con un producto físico seleccionado en modo real (botón "Pagar $24.30 con ONVO Pay", sin ningún texto de demostración) y en inglés (mismo comportamiento, textos traducidos) — confirmando visualmente que el cambio se ve correctamente en los 3 casos y que el resto de la página (menú, testimonios, WhatsApp flotante, etc.) no se alteró.

**Pendiente para terminar de "poner a funcionar" el sitio (checklist conversado con el usuario):** (a) reemplazar en Netlify las llaves de prueba de ONVO por las de producción que el usuario ya tiene — esto lo debe hacer el propio usuario en el panel de variables de entorno de Netlify, nunca ingresando esas llaves a través de esta conversación; (b) subir a producción todo el trabajo ya aprobado y aún no desplegado, incluyendo este cambio de texto y el del punto 62 (pantalla de marca + botón de llamada); (c) confirmar en Resend si el dominio `vidavitalqr.com` está verificado, para asegurar que los correos de confirmación/código lleguen sin problema; (d) enviar a ONVO el documento de respaldo que piden para las liquidaciones (ver arriba); (e) lanzar por ahora solo con pago en dólares (ya probado de punta a punta) y dejar CRC/SINPE Móvil para después, hasta confirmarlos por separado (ver punto 58 y el punto 17 de pendientes).

## 64. Primer pago real en producción — confirmado funcionando, y corrección del sonido de la pantalla de marca (2026-09-24)

Siguiendo el checklist de lanzamiento del punto 63, el usuario reemplazó en Netlify las variables `ONVO_SECRET_KEY` (llave secreta de producción de ONVO, marcada como "Contains secret values") y `ONVO_WEBHOOK_SECRET` (tras crear un webhook nuevo en modo Live en el panel de ONVO, apuntando a `https://vidavitalqr.com/.netlify/functions/onvo-webhook`), subió el ZIP más reciente a través de su repositorio de GitHub conectado a Netlify (su forma habitual de desplegar: sube los archivos desde la interfaz web de GitHub, lo que dispara el despliegue automático en Netlify), y confirmó que el checkout público ya no muestra ningún texto de "Mockup".

**Primera compra real de prueba, hecha por el propio usuario con sus datos reales** ("Código QR — solo digital — Personal", $7.00 + IVA): de punta a punta funcionó correctamente —

- El pago se cobró en colones (el cliente eligió esa moneda), con el tipo de cambio automático del BCCR (+2%, redondeado hacia arriba) aplicado correctamente: equivalente a $7.91 cobrado a ₡464/dólar.
- Llegó el correo de aviso de pago real al administrador con todos los datos esperados (producto, sesión de ONVO, modo "live", monto en colones y su equivalente en dólares).
- La ficha creada se ve correctamente: foto, datos, "Información de emergencia", botón "Llamar" con el número enmascarado, y el aviso de autorización de ubicación exacta — todo funcionando.
- El selector de idioma (ES/EN/FR/PT) en la ficha funciona correctamente: se confirmó que, aunque la ficha se llenó en español, cualquiera que la escanee puede cambiar el idioma con un clic y el contenido se traduce bien (se confirmó específicamente con francés). Aclarado con el usuario: esto es el comportamiento ya definido a propósito (ver punto 56) — arranca en español para cualquier visitante nuevo, y respeta el idioma elegido si la misma persona vuelve a escanear otra ficha desde el mismo navegador.

**Único problema real encontrado: no sonaba la campanita al abrir la ficha** (el logo sí aparecía). Causa identificada: los navegadores móviles (confirmado por el usuario con Android, y aplica igual en iPhone) crean el "contexto de audio" en estado `suspended` hasta detectar una interacción directa del usuario, y el código nunca le pedía explícitamente que se reanudara (`resume()`) — por eso los sonidos se programaban pero nunca llegaban a escucharse, sin ningún error visible. Se corrigió en `netlify/functions/ver.js`: ahora, si el contexto de audio arranca suspendido, se le pide reanudarse y el sonido se reproduce justo después de que eso ocurra; si ya está listo, suena de inmediato como antes. Si el navegador de todas formas bloquea el audio (ej. iPhone en modo silencio), sigue sin sonar sin afectar el resto de la ficha, exactamente como estaba documentado. Cambio de una sola pieza de código, verificado con diff completo contra el respaldo (solo se modificó esa función) y con una prueba de renderizado que confirma que el resto de la ficha (botón de llamada, aviso de ubicación) sigue intacto.

**Pendiente de subir a producción:** este cambio de `netlify/functions/ver.js` (corrección del sonido) todavía no se ha desplegado — falta que el usuario suba el ZIP más reciente por su método habitual (GitHub → Netlify).

## 65. PDF de la ficha disponible en los 4 idiomas al escanear, y contador de fichas reiniciado a partir de 500 (2026-09-24)

El usuario reportó que, al escanear una ficha y cambiar el idioma con el selector (ES/EN/FR/PT), la pantalla del visor sí se traducía correctamente, pero el botón para descargar el PDF siempre abría el documento en español (el idioma con el que se llenó originalmente), sin importar el idioma elegido. Aclaró explícitamente que el idioma de llenado y el idioma de escaneo son dos cosas independientes, y que quien escanea debe poder elegir el idioma también para el PDF.

**Causa:** el PDF no se genera traduciendo texto — se genera tomando capturas de pantalla del formulario ya lleno (con `html2canvas`) y armando el PDF a partir de esas imágenes (con `jsPDF`). Por eso solo existía en el idioma con el que se llenó el formulario; no hay forma de "traducir" un PDF ya generado, hay que volver a renderizar el formulario en cada idioma y capturarlo de nuevo.

**Solución implementada** (diseñada para no arriesgar el flujo de guardado ya probado — el usuario pidió explícitamente que cualquier posible falla en esto "funcione como un reloj suizo" sin afectar el resto):

- Después de que la ficha se guarda con éxito de la forma normal (igual que siempre, sin cambios), el formulario cambia su propio idioma internamente, en los otros 3 idiomas restantes, uno por uno; genera un PDF nuevo en cada uno con la misma función que ya existía; y sube cada PDF por separado a una función nueva y pequeña (`netlify/functions/subir-pdf-idioma.js`), que solo agrega ese PDF a la ficha que ya se guardó (sin tocar foto, contactos, tarjeta, correo ni nada más).
- Cada idioma se sube de forma independiente: si uno falla (por conexión, tamaño, lo que sea), los demás no se ven afectados, y la ficha principal — que ya se guardó exitosamente al inicio — nunca se ve afectada por estos fallos.
- En el visor (`ver.js`), cuando la persona que escanea cambia de idioma, el botón de PDF ahora apunta al PDF de ese idioma si logró subirse; si ese idioma en particular no se pudo generar por algún motivo, el botón simplemente cae de vuelta al PDF original, sin mostrar ningún error a quien escanea.
- Efecto secundario esperado y aceptado por el usuario: el formulario tarda un poco más en terminar de procesar después de guardar la ficha, porque genera los 3 PDFs adicionales antes de mostrar el PIN o continuar al pago.

Aplicado igual en los tres tipos de ficha (personas, mascotas, objetos). Verificado en los 3 archivos HTML y en `ver.js`: sintaxis validada con `node --check` y comparación línea por línea contra el respaldo previo, confirmando que solo se tocaron exactamente los bloques nuevos (nada más del formulario ni del visor cambió).

**Contador de fichas reiniciado a partir de 500:** el usuario pidió que el contador de fichas realizadas (visible en la página de creación) arrancara en 500 desde este momento, tanto para dar la impresión de que ya existen 500 fichas como para poder distinguir de ahora en adelante las fichas de la etapa de pruebas de las reales. El contador lo provee un servicio externo gratuito (abacus.jasoncameron.dev) que no tenía una llave de administrador configurada, así que no era posible simplemente "poner" el número en el contador existente. Se investigó y se confirmó una alternativa: el mismo servicio permite crear un contador nuevo ya con un valor inicial elegido, sin necesitar la llave que nunca se configuró. El propio usuario creó los 3 contadores nuevos (uno por tipo de ficha) mediante ese método, cada uno en 499 (de forma que la siguiente ficha de cada tipo será la #500), y se actualizó `COUNTER_NAMESPACE` en `ficha.html`, `ficha-mascota.html` y `ficha-objeto.html` para apuntar a esos contadores nuevos.

**Pendiente:** falta que el usuario suba estos archivos (subir-pdf-idioma.js nuevo, ver.js, ficha.html, ficha-mascota.html, ficha-objeto.html) por su método habitual (GitHub → Netlify), y que haga una prueba real (llenando y guardando una ficha) para confirmar que el PDF en los otros idiomas se genera y se ve correctamente, y que el contador de fichas arranca en 500.

## 66. Corrección real del sonido de la pantalla de marca — el arreglo del punto 64 no era suficiente (2026-09-24)

El usuario confirmó, tras subir el arreglo del punto 64 a producción, que el idioma de la ficha ya funcionaba perfecto pero **el sonido seguía sin escucharse**. Pidió además una forma de probar los cambios sin subirlos cada vez a Netlify, para no gastar créditos de más.

**Causa real (más precisa que el diagnóstico anterior):** se armó una prueba automatizada, fuera de Netlify, cargando la página del visor en un navegador Chromium real (headless) y verificando el estado del motor de audio del navegador. La prueba confirmó que **abrir el enlace que resulta de escanear un código QR no cuenta, en la práctica, como una interacción directa del usuario** para el navegador — por eso el `ctx.resume()` agregado en el arreglo anterior nunca lograba reactivar el audio: quedaba "suspendido" indefinidamente porque nunca hubo un toque/clic real dentro de la página misma. El diagnóstico del punto 64 (motor de audio suspendido) era correcto, pero la solución (pedir que se reanude solo al cargar) no bastaba.

**Corrección aplicada en `netlify/functions/ver.js`:** además del intento inmediato al cargar la página (que sigue ahí, por si algún navegador sí lo permite), ahora también se reintenta reproducir el sonido en el primer toque, clic o tecla que la persona haga en cualquier parte de la pantalla — algo que de todas formas suele pasar rápido (para cambiar de idioma, responder el aviso de ubicación, etc.). Se agregó una protección para que el sonido nunca se repita si hay más de un toque. El splash visual (logo apareciendo y desapareciendo) sigue funcionando exactamente igual que antes, sin depender de si hubo sonido o no.

**Cómo se probó, sin usar Netlify:** se generó localmente el HTML exacto que produce `ver.js` para una ficha de prueba, y se cargó en un navegador Chromium real (el mismo motor que usa Chrome) controlado por un script de prueba, sin publicar nada. Se verificó automáticamente: (1) que sin ningún toque, el sonido correctamente NO suena y no hay errores (reproduce el reporte del usuario); (2) que con un toque después de cargar, el sonido SÍ suena; (3) que con dos toques seguidos, el sonido suena una sola vez, no se duplica; (4) que el splash visual (logo) sigue apareciendo y desapareciendo en los mismos tiempos de antes. Los cuatro resultados fueron los esperados. Además, diff completo contra la versión anterior del archivo confirmando que el único bloque modificado es el de la pantalla de marca — nada más de la ficha (ubicación, contactos, PDF por idioma) se tocó.

**Nota para futuras pruebas similares:** este tipo de verificación (cargar la página real en un navegador de prueba, simular toques y revisar el resultado) se puede repetir sin publicar nada a Netlify cada vez que haya dudas sobre un comportamiento del navegador — solo hace falta el archivo de la función y datos de ejemplo.

**Confirmado por el usuario (2026-09-24):** tras subir el ZIP, probó en un celular real (con fichas ya existentes, creadas antes de este arreglo) y confirmó que el sonido ya suena al tocar la pantalla, y que funciona igual con cualquier ficha ya creada — como se esperaba, ya que la lógica vive en `ver.js` (se ejecuta al escanear, no depende de cuándo se creó la ficha). Punto cerrado.

## 67. Confirmado: SINPE Móvil sí está activo en el checkout real de ONVO Pay (2026-09-24)

James preguntó si el pago con SINPE Móvil estaba activado en el checkout de ONVO Pay. Se revisó el código (`netlify/functions/crear-pago.js`): la función solo crea una sesión de checkout alojada por ONVO (`checkout/sessions/one-time-link`) y redirige ahí al cliente — **no se envía ningún parámetro de método de pago**, así que qué opciones aparecen depende enteramente de la configuración de la cuenta de ONVO Pay, no del código del sitio. De paso se detectó que la pestaña "SINPE Móvil" del checkout de nuestro propio sitio (con el campo de número de teléfono) es solo visual/informativa — ese número nunca se envía a ningún lado, ni a ONVO ni a ninguna función nuestra; el pago real siempre ocurre en la página de ONVO.

James confirmó con una captura de su panel de ONVO Pay (sección "Métodos de pago", modo Live): **Tarjeta de crédito o débito: Activo**, **SINPE Móvil: Activo**, Google Pay: apagado (requiere tener tarjeta activa, que ya la tiene, para poder activarse). Con esto queda confirmado que SINPE Móvil sí debería estar disponible para los clientes en el checkout real, sin necesidad de ningún cambio de código. Pendiente de vieja data (`revision-mercado-vidavitalqr.md`) sobre si el cobro real por SINPE ya funcionaba: con esta confirmación de la cuenta, se da por resuelto — solo faltaría, si James quiere, una prueba real pagando específicamente por esa vía para confirmarlo de punta a punta (no es obligatorio, ya que la cuenta lo tiene activo).

## 68. SINPE Móvil real: pago manual por WhatsApp (no automatizado vía ONVO) (2026-09-24)

Tras confirmar (punto 67) que SINPE Móvil está "Activo" en la cuenta de ONVO Pay pero que en la práctica nunca aparecía como opción para el cliente (el checkout real solo mostraba "Pagar con ONVO Pay" porque `index.html` ocultaba por completo las pestañas de método de pago en modo real), James explicó cómo funciona el validador de SINPE Móvil de ONVO: requiere que el comprador comparta cédula y teléfono dentro del link de pago, y confirma la transacción por su cuenta — es una integración separada de las sesiones de checkout genéricas que usa `crear-pago.js`, con parámetros/endpoint de creación que la documentación pública de ONVO no deja ver completos (solo se pudo confirmar un endpoint de "listar pagos por SINPE Móvil", no uno de "crear").

Dado el riesgo de implementar a ciegas una integración de pagos mal documentada, se le presentó a James la alternativa de un flujo **manual**: el checkout muestra el monto en colones y un número de SINPE, el cliente paga desde su propia aplicación bancaria y avisa por WhatsApp (canal que el negocio ya usa activamente), y James confirma cada pago a mano. James eligió explícitamente esta opción sobre la automatizada vía ONVO. Se le presentó una maqueta visual (HTML, con los mismos colores y tipografías del sitio real) para su aprobación antes de tocar el código de producción, incluyendo un recuadro que le recuerda al cliente adjuntar la captura del comprobante en el chat de WhatsApp (WhatsApp no permite adjuntar archivos automáticamente desde un link — es una limitación técnica de WhatsApp, no del sitio). James aprobó la maqueta y dio luz verde para implementarlo ("Me parece bien el proceso como lo describes. procedamos con el ajuste.").

**Implementado en `index.html`** (ningún otro archivo necesitó cambios — no hay backend nuevo, es 100% manual):

- Las pestañas "Tarjeta" / "SINPE Móvil" del checkout ahora se muestran también en modo real (antes se ocultaban por completo). La pestaña "SINPE Móvil" solo se ofrece si ya se cargó el tipo de cambio (el monto de SINPE siempre se muestra en colones, sin importar si el cliente tiene elegido $ o ₡ en el selector de moneda); si el tipo de cambio no está disponible, se cae de vuelta a solo "Tarjeta", igual que antes.
- Al elegir la pestaña "Tarjeta" en modo real, el comportamiento es exactamente el de siempre: botón "Pagar × con ONVO Pay", que sigue redirigiendo a la página hospedada de ONVO Pay vía `crear-pago.js` — sin ningún cambio.
- Al elegir la pestaña "SINPE Móvil" en modo real, aparece un recuadro nuevo con: el monto total en colones, el número al que enviar el SINPE (**6093-3090**, con botón "Copiar número"), los pasos a seguir, un aviso destacado de adjuntar el comprobante, y un botón verde "Ya pagué — avisar por WhatsApp". Ese botón valida que el cliente haya escrito su correo, arma un mensaje ya redactado (monto, producto(s), folio(s), correo y el recordatorio del comprobante) y abre WhatsApp (`wa.me/50660933090`) en una pestaña nueva. **No se llama a `crear-pago.js` ni a ninguna función de ONVO en este flujo** — es enteramente manual, tal como se aprobó.
- El bloque anterior de SINPE del mockup demo (`#cxFieldsSinpe`, un campo de teléfono cosmético que nunca se usaba realmente) se dejó intacto y sigue funcionando igual en el modo demo (sin folio) — el bloque nuevo es independiente y solo aparece en modo real.
- Los textos nuevos usan el mismo patrón de traducción que el resto del sitio (`data-i18n`, con el texto en español como respaldo); por ahora no se agregaron las claves nuevas al diccionario de EN/FR/PT, así que un visitante en esos idiomas vería este bloque en español — pendiente de traducir si James lo quiere.
...
- **Configuración de redes sociales (Facebook, Instagram y TikTok) — en curso (ver puntos 61-62):** WhatsApp Business, Facebook e Instagram ya quedaron completos y conectados entre sí; solo falta TikTok, apoyándose en la guía ya entregada `VidaVitalQR_Guia_Redes_Sociales.docx`.
- Preparar y publicar videos cortos para redes sociales usando el concepto ya aprobado de "familiarización con el escaneo" (logo + campanita suave extendida) del punto 62.
...
- **Pago en colones (ver punto 58):** el tipo de cambio automático (BCCR + 2% + redondeo hacia arriba) ya está confirmado funcionando en producción, usando la API nueva del BCCR (SDDE). Falta únicamente: una transacción de prueba real en CRC con ONVO Pay para confirmar el multiplicador correcto (`ONVO_CRC_SUBUNIT_MULTIPLIER`) antes de poner `HABILITAR_PAGO_CRC=true`.
- **Subir a producción (GitHub/Netlify) todo el trabajo ya aprobado y pendiente de subir:** `admin-envios.html` con las correcciones combinadas de los puntos 50 y 51; `ficha.html`, `ficha-mascota.html`, `ficha-objeto.html` con el aviso "Procesando su ficha…" del punto 52, (en `ficha.html`) el Identificador QR agrandado del punto 55, y (en las 3 fichas) el idioma por defecto en español del punto 56; del punto 53/54/55: `netlify/functions/avisar-escaneo.js` (enlace de ubicación corregido + correo en el idioma de la ficha), `netlify/functions/send-ficha.js` (idioma guardado + correo de código traducido), `netlify/functions/crear-pago.js` y `netlify/functions/onvo-webhook.js` (costo de envío de $8.50), `index.html` (~55 textos adicionales traducidos + costo de envío en el mockup), y `admin-opiniones.html` (opción de idioma); del punto 56: `index.html` (hero reordenado como portada, corrección de "Acero inoxidable", imágenes de muestra por idioma, indicador de scroll del menú, idioma por defecto en español), `netlify/functions/ver.js` (idioma por defecto en español) y `admin-opiniones.html` (idioma por defecto en español, sumado a la opción de idioma del punto 54); del punto 57: `index.html` (nota de $8.50 junto a la línea de envío del carrito, nuevo recuadro de aclaración de envío en "Presentaciones", y corrección de los dos textos que quedaban contradictorios con la política de envío); y del punto 62: `netlify/functions/ver.js` (pantalla de marca con logo real + sonido "campanita suave extendida" sintetizado por código al abrir la ficha, botón "Llamar" con número enmascarado junto a cada contacto de emergencia, y traducción de "Llamar" en los 4 idiomas — sin cambios en la lógica de ubicación ni en el aviso por correo, ver detalle y verificación en el punto 62); y del punto 63 (más reciente): `index.html` (se retiró/actualizó el texto "Mockup de checkout — solo demostración" del recuadro de pago en los 4 idiomas, y se corrigió que el aviso interno de repaso ya no aparezca por defecto al cargar la página — sin cambios en la lógica de pago, ver detalle y verificación en el punto 63). Mientras no se suban, el sitio en vivo sigue sin ninguno de estos cambios. Pendiente confirmar, ya en producción: que el enlace de ubicación del correo de aviso de escaneo vuelve a ser clickeable, que los correos automáticos llegan en el idioma correcto según lo elegido al llenar la ficha, que la ficha pública (`ver.js`) muestra el texto ya traducido correctamente (punto 55, observación 1), que el sitio arranca en español para un visitante nuevo (punto 56), y que la aclaración de gastos de envío ya es visible en el carrito y en "Presentaciones" (punto 57).
